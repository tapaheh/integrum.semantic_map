(function ( $ ) {
    $.fn.semanticMap = function( options ) {
        var settings = $.extend({
            ...
            image1:'./img/ico_person.png',
            image2:'./img/ico_org.png'
        }, options );
        ...
        if (settings.paperHeight == 'auto') {
            ...
            while (parent.height() < settings.paperMinSize && parent.prop('tagName') != 'BODY') {
                svgPadding += parent.outerHeight(true) - parent.height();
                parent = parent.parent();
            }
            paperHeight = parent.innerHeight() < paperHeight ? paperHeight : parent.height()-svgPadding;
        } else {
            paperHeight = settings.paperHeight;
        }

        paperHeight = (paperHeight > paperWidth) ? (paperWidth-svgPadding*2) : paperHeight;
